				...
				if getsynasset then
					...
					elseif (#image > 14) and (string.sub(image, 1, 14) == "synasseturl://") then
						local x, e = pcall(function()
							...
							local fold = isfolder("./cware")
							if fold then
							else
								makefolder("./cware")
							end
							fold = isfolder("./cware/Themes")
							if fold then
							else
								makefolder("./cware/Themes")
							end
							fold = isfolder("./cware/Themes/SynapseAssetsCache")
							if fold then
							else
								makefolder("./cware Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./cware/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./cware/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./cware/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "cware" then
		library.WorkspaceName = convertfilename(windowName, "cware")
	end
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./cware") then
						makefolder("./cware")
					end
					local common_string = "./cware/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./cware/" .. tostring(custom_workspace or library.WorkspaceName)
						end
